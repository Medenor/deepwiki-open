...
def get_codeberg_username(access_token: str) -> Optional[str]:
    """Resolve the Codeberg username associated with a token.

    The Forgejo API requires Basic Auth with a username and token (as password) for
    cloning private repositories. This helper queries the `/user` endpoint to fetch
    the login name so we can embed it in the clone URL.
    """

    api_url = "https://codeberg.org/api/v1/user"
    ...
    try:
        ...
        if not username:
            logger.warning("Codeberg token resolved but no username returned from API")
        return username
    except RequestException as exc:
        logger.warning("Failed to resolve Codeberg username: %s", exc)
        return None
    except ValueError as exc:
        # JSON parsing errors inherit from ValueError
        logger.warning("Failed to parse Codeberg user response: %s", exc)
        return None
